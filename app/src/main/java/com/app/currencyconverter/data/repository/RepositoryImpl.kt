...
import com.app.currencyconverter.data.remote.*
...
class RepositoryImpl @Inject constructor(
    ...
    @WorkerThread
    override suspend fun getAllCurrencies(): Flow<DataState<CurrenciesModel>> {
        return flow {
            apiService.getAllCurrencies().apply {
                this.onSuccessSuspend {
                    data?.let {
                        emit(DataState.success(it))
                    }
                }.onErrorSuspend {
                    emit(DataState.error<CurrenciesModel>(message()))

                }.onExceptionSuspend {
                    if (this.exception is IOException) {
                        emit(DataState.error<CurrenciesModel>(stringUtils.noNetworkErrorMessage()))
                    } else {
                        emit(DataState.error<CurrenciesModel>(stringUtils.somethingWentWrong()))
                    }
                }
            }
        }
